...
def gen_static_friction(s, scaling=1e3, x0n=-0.01e-3, x1n=0.01e-3,plot=False):
    ...
    # x1n = 1e-6
    mid_point = (x0n+x1n)/2
    y_constant = -(sympy.tanh(scaling * (mid_point - x1n * 2)) - sympy.tanh(scaling * (mid_point - x0n * 2)))
    y = (sympy.tanh(scaling * (x - x1n*2)) - sympy.tanh(scaling * (x - x0n*2)))/y_constant+1

    if plot:
        xn = numpy.r_[-.5:.5:1001j]
